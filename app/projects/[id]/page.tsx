    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              {project.milestones.map((m, i) => (
                <div key={i} className="flex items-center gap-3">
                  {m.done ? (
                    <CheckCircle2
                      size={17}
                      style={{ color: "#6633ee", flexShrink: 0 }}
                    />
                  ) : (
                    <Circle
                      size={17}
                      style={{ color: "#1e1e2e", flexShrink: 0 }}
                    />
                  )}
                  <span
                    className="text-sm"
                    style={{ color: m.done ? "#c4c4d4" : "#8b8b9e" }}
                  >
                    {m.title}
                  </span>
                </div>
              ...
              {project.updates.map((u, i) => (
                <div key={i} className="flex gap-4 pb-5 relative">
                  ...
                  {i < project.updates.length - 1 && (
                    <div
                      className="absolute left-[7px] top-5 w-px h-full"
                      style={{ backgroundColor: "#1e1e2e" }}
                    />
                  )}
                  <div
                    ...
                    style={{
                      background:
                        i === 0
                          ? "linear-gradient(135deg, #6633ee, #a855f7)"
                          : "#1e1e2e",
                      border:
                        i !== 0 ? "2px solid #2e2e44" : "none",
                    }}
                  />
                  <div>
                    <span
        ...
        <div className="flex flex-col gap-4">
              ...
              {teamMembers.map((member) =>
                member ? (
                  ...
                  >
                    ...
                    />
                    <div>
                      <p className="text-xs font-medium text-white hover:underline">
                        {member.name}
                      </p>
                      <p className="text-xs" style={{ color: "#8b8b9e" }}>
                        {member.major.split(" ")[0]}
                      </p>
                    </div>
                  </div>
                ) : null
          ...
          >
            ...
            <button
              onClick={handleApply}
              className="mt-4 w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white transition-opacity hover:opacity-90"
              style={{
      ...
      {showToast && (
        ...
        >
          ...
          Application submitted! The team will reach out soon.
        </div>
      )}
    </div>
  );
