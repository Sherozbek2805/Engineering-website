...
import { getDirectMessages, sendDirectMessage, getProfileById, type DbDirectMessage, type ProfileSnippet } from "@/lib/db";
...
export default function DMThreadPage() {
  ...
  const [other, setOther] = useState<ProfileSnippet | null>(null);
  ...
  async function loadData() {
    const [profile, msgs] = await Promise.all([
      ...
      currentUser ? getDirectMessages(currentUser.id, otherId) : Promise.resolve([]),
    ]);
    setOther(profile);
    setMessages(msgs);
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    setSending(true);
    await sendDirectMessage(currentUser.id, otherId, input.trim());
    setInput("");
    setSending(false);
    await loadData();
  }
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <div className="px-5 py-3 flex items-center gap-3 flex-shrink-0" style={{ borderBottom: "1px solid #1e1e2e" }}>
        ...
        <VerifiedAvatar name={other.name} avatarUrl={other.avatar_url} verified={other.verified} size="sm" />
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-0.5">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 h-full text-center">
            <VerifiedAvatar name={other.name} avatarUrl={other.avatar_url} verified={other.verified} size="xl" />
        ...
        {messages.map((msg, i) => {
          const authorProfile = msg.from_profile;
          ...
          return (
            <div key={msg.id} className="flex gap-3 mt-3 pr-2 py-1 rounded hover:bg-[#0f0f18] -mx-2 px-2">
              <VerifiedAvatar name={authorProfile.name} avatarUrl={authorProfile.avatar_url} verified={authorProfile.verified} size="sm" />
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline gap-2 mb-0.5">
                  <span className="text-sm font-semibold text-white">{authorProfile.name}</span>
